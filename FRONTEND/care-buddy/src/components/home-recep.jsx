...
export default function Home() {
    ...
    const fetchAppointments = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8082/appointment-service/appointments/today"
        );
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8082/appointment-service/appointments/count"
        );
  ...
  //fetch Pets count
    ...
    const fetchCount = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8084/pet-service/pets/count"
        );
      ...
      <Row>
        <Col md={4} className="mb-3">
          <Card className="dashboard-card" style={{ borderLeft: "6px solid #1E3A5F" }}>
            <h5 style={{ color: "#1E3A5F" }}>Total Pets</h5>
            <h2>{petCount}</h2>
          </Card>
        </Col>
        ...
        <Col md={4} className="mb-3">
          <Card className="dashboard-card" style={{ borderLeft: "6px solid #1E3A5F" }}>
            <h5 style={{ color: "#1E3A5F" }}>Upcoming Appointments</h5>
            <h2>{aptCount}</h2>
          </Card>
        </Col>
        ...
        <Col md={4} className="mb-3">
          <Card className="dashboard-card" style={{ borderLeft: "6px solid #1E3A5F" }}>
            <h5 style={{ color: "#1E3A5F" }}>Available Doctors</h5>
            <h2>{doctorCount}</h2>
          </Card>
        </Col>
      </Row>
